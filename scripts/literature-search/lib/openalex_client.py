...
Rate limits (with polite pool email):
  - 10,000 requests/day (most generous of the three sources)
"""
...
_POLITE_EMAIL = "research@example.com"  # 替换为你的邮箱可进入 polite pool
_MAX_PER_REQUEST = 50
_MAX_RETRIES = 3
_MAX_WAIT_SEC = 60
_TIMEOUT_SEC = 20
...
def search_openalex(
    ...
    *,
    ...
    email : str
        Your email for OpenAlex polite pool (higher rate limits).
        Set to your own email for best results.
    """
...
def _request_with_retry(url: str, email: str) -> dict[str, Any] | None:
        ...
        try:
            req = urllib.request.Request(
                ...
                headers={
                    ...
                    "User-Agent": f"LiteratureSearch/1.0 (mailto:{email})",
                },
            )
            ...
            if exc.code == 429:
                ...
                logger.warning("OpenAlex 429. Waiting %.1fs (attempt %d/%d)...", wait, attempt + 1, _MAX_RETRIES)
            ...
            logger.warning("OpenAlex request failed (%s). Retry %d/%d", exc, attempt + 1, _MAX_RETRIES)
            time.sleep(wait + random.uniform(0, wait * 0.2))
